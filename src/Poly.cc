...
/*
  ...
  For efficiency, implementation also performs a couple
  of quick (incomplete) tests using bounding boxes and
  affine dimension.
*/
...
Poly_Impl::closed_join_assign_if_exact(const Poly_Impl& y) {
  ...
  // For disjoint closed polyhedra, hull is not exact:
  // quick (incomplete) check based on bounding boxes.
  ...
  // only when inclusion holds
  // (note: check only valid for closed polyhedra).
  ...
  auto subsumed_by = [](const auto& gs, const auto& ph) {
    std::pair<Bits, Bits> res;
    for (auto i : bwd_index_range(gs.sing_rows)) {
      if (ph.relation_with(gs.sing_rows[i])
          .implies(Poly_Gen_Rel::subsumes()))
        res.first.set(i);
    }
    for (auto i : bwd_index_range(gs.sk_rows)) {
      if (ph.relation_with(gs.sk_rows[i])
          .implies(Poly_Gen_Rel::subsumes()))
        res.second.set(i);
    }
    return res;
  ...
  const auto& [sing_sub1, sk_sub1] = subsumed_by(gs1, ph2);
  auto num_sub1 = sing_sub1.size() + sk_sub1.size();
  ...
  const auto& [sing_sub2, sk_sub2] = subsumed_by(gs2, ph1);
  auto num_sub2 = sing_sub2.size() + sk_sub2.size();
  ...
  if (num_rows(cs1.sing_rows) > 0) {
    ...
    bool C_holds
      = (sing_sub1.size() < num_rows(gs1.sing_rows))
  ...
  if (num_rows(cs1.sk_rows) > 0) {
    ...
    bool C_holds_for_a_line = (sing_sub1.size() < num_rows(gs1.sing_rows));
  ...
  // (and all gens in sing_sub1/sing_sub2 are redundant).
